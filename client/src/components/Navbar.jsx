...
function Navbar() {
  ...
  const {user,getCartCount,axios, setUser, showUserLogin, setShowUserLogin, navigate,setSearchQuery,searchQuery} = useAppContext()
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink to='/' onClick={()=>setOpen(false)}>
                <p className='text-2xl font-bold'><span className='text-[#4fbf8b] f'>S</span>okoni</p>
            </NavLink>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                { !user ? (<button className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full" onClick={()=>setShowUserLogin(true)}>
                    Login
                </button>) : (
                    <div className='relative group'>
                     <img src={assets.profile_icon} className = 'w-10 cursor-pointer' alt=''/>
                     <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
                        <li className='p-1 pl-3 hover:bg-primary/10 cursor-pointer' onClick={()=> navigate('my-orders')}>My Orders </li>
...
<div className='flex items-center gap-6 sm:hidden'>
    ...
    <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="">
           ...
           {open && (  <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-30`}>
                ...
                {
                    !user ? (
<button className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-primary-dull transition text-white rounded-full text-sm" onClick={()=>{setOpen(false);
